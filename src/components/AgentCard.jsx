...
export default function AgentCard({ agent }) {
  ...
  const provLabel =
    providerLabels[agent?.provider] || agent?.provider || "Any Provider";
  ...
  return (
    <>
      ...
      >
        ...
        {/* Bottom: provider badge + reliability badge + run link */}
        <div className="flex items-center justify-between mt-auto gap-2 flex-wrap">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <ReliabilityBadge agentId={agent.id} />
          </div>
          <span className="flex items-center gap-1 text-xs font-medium text-accent opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0 group-focus-visible:translate-x-0">
            ...
            <ArrowRight
              size={12}
              className="transition-transform duration-300 transform group-hover:translate-x-1 group-focus-visible:translate-x-1"
            />
          </span>
        </div>
      </Link>
